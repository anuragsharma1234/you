...
MAX_ATTACK_TIME = 300  # Example: Default maximum attack time set to 300 seconds
# In-memory storage
users = {}
keys = {}
last_attack_time = {}
...
def create_random_key():
    key = "VILAX-" + ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(10))
...
def start_command(message):
    ...
    bot.reply_to(message, "𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗧𝗢 𝗢𝗙𝗙𝗜𝗖𝗜𝗔𝗟™ 𝗦𝗘𝗥𝗩𝗘𝗥", reply_markup=markup)
    ...
    if user_id in users and users[user_id]:  # Ensure user has valid expiration
        ...
        if user_id in last_attack_time:
            ...
            if time_since_last_attack < COOLDOWN_PERIOD:
                ...
                response = f"⌛  𝗖𝗼𝗼𝗹𝗱𝗼𝘄𝗻 𝗶𝗻 𝗲𝗳𝗳𝗲𝗰𝘁 𝘄𝗮𝗶𝘁 {int(remaining_cooldown)} 𝘀𝗲𝗰𝗼𝗻𝗱𝘀"
    ...
    if len(details) == 3:
        ...
        try:
            ...
            if attack_time > MAX_ATTACK_TIME:
                response = f"❗  Error: Maximum allowed attack time is {MAX_ATTACK_TIME} seconds!"
            else:
